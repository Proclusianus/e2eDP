...
class DBManager:
    ...
    def get_all_transaction_types(self):
        """Returns a list of enum values labels (strings)"""
        query = """
        ...
        """
        return pd.read_sql(query, self.engine)['enumlabel'].tolist()
        
    def get_all_market_types(self):
        """Returns a list of enum values labels (strings)"""
        query = """
            SELECT enumlabel FROM pg_enum
            JOIN pg_type ON pg_enum.enumtypid = pg_type.oid
            WHERE pg_type.typname = 'market_type_enum'
            ORDER BY enumsortorder;
        """
        return pd.read_sql(query, self.engine)['enumlabel'].tolist()
    ...
    def get_anomaly_analysis_definitions(self):
        """Returns a list of record dictionaries (entire record)"""
